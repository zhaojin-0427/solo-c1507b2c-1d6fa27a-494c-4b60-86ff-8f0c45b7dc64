...
def _leaves(pages: int, total: int = 999) -> list[tuple]:
    """从 0 基页码起的顺序叶页码表。"""
    return make_leaf_pages(pages // 2, 0, total)
...
def test_layout_pages_complete(pages, style, binding):
    ...
    layout = generate_layout(cols, rows, prog, _leaves(pages))
...
def test_reading_order_and_orientation(pages, style, binding):
    ...
    prog = build_program(cols, rows, binding, style)
    layout = generate_layout(cols, rows, prog, _leaves(pages))
    ver = verify_layout(layout, prog, _leaves(pages))
    assert ver["ok"], ver
...
def test_blank_pages_at_end(pages, style, binding):
    ...
    prog = build_program(cols, rows, binding, style)
    layout = generate_layout(cols, rows, prog, _leaves(pages, total))
    ver = verify_layout(layout, prog, _leaves(pages, total))
    assert ver["ok"]
    seq = []
    for leaf in ver["reading_order"]:
        seq += [leaf["recto"], leaf["verso"]]
...
def test_known_8page_layout():
    ...
    layout = generate_layout(2, 2, prog, _leaves(8))
